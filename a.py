...
def get_msg():
    ...
    gradeli = doc("div.description").items()
    """
            ...
            """
    s_dict_svg_text, s_list_svg_y, s_dict_css_x_y = s_css_get(doc)

    # 解析每条评论
    pinglunLi = doc("div.singleUser").items()

    dict_svg_text, list_svg_y, dict_css_x_y = css_get(doc)

    for data in pinglunLi:
        pinglun = data("div.comment-entry").html()
        print("", css_decode(dict_css_x_y, dict_svg_text, list_svg_y, pinglun))
        print("*" * 10)
    for data in gradeli:
        grade = data("div.description").html()
        print("", s_css_decode(s_dict_css_x_y, s_dict_svg_text, s_list_svg_y, grade))
...
def s_css_get(doc):
    ...
    s_dict_avg_text, s_list_svg_y = s_svg_text(svg_link)
    """
    css_dict() 方法：生成css样式中background的样式库
    dict_css: 返回css字典样式
    """
    s_dict_css = s_css_dict(background_link.text)
    return s_dict_avg_text, s_list_svg_y, s_dict_css


# 2-字体库链接
def s_svg_text(url):
    html = requests.get(url)
    s_dict_svg, s_list_y = s_svg_dict(html.text)
    return s_dict_svg, s_list_y


# 3-生成svg字库字典
def s_svg_dict(csv_html):
    ...
    for data in svg_text_re:
        s_dict_avg[data[0].replace("#", "")] = list(data[2])
    ...
    for data in svg_y_re:
        s_list_y.append([data[0], data[2]])
    return s_dict_avg, s_list_y
...
def s_css_dict(html):
    ...
    for data in css_text_re:
        ...
        """
        s_dict_css[data[0]] = (data[1], data[2], x, data[2])
    return s_dict_css


# 5-最终评论汇总
def s_css_decode(css_html, svg_dict, svg_list, pinglun_html):
    """
...
def css_get(doc):
    ...
    r = r'background-image: url(.*?);'
    matchObj = re.compile(r, re.I)
    svg_link = matchObj.findall(background_link.text)[3].replace(")", "").replace("(", "http:")
    """
